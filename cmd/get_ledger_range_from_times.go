...
func init() {
	...
	getLedgerRangeFromTimesCmd.Flags().String("datastore-type", "GCS", "Datastore type containing LedgerCloseMetaBatch files used for the binary search over close times. Accepted values: GCS, S3.")
	getLedgerRangeFromTimesCmd.Flags().String("datastore-region", "", "Datastore region. Required when --datastore-type is S3.")
	getLedgerRangeFromTimesCmd.Flags().String("datastore-endpoint-url", "", "Optional datastore endpoint URL. Used with --datastore-type S3.")
