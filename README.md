...
These commands export information using the [Ledger Exporter](https://github.com/stellar/go/blob/master/exp/services/ledgerexporter/README.md) output files within a specified datastore ([datastore](https://github.com/stellar/go/tree/master/support/datastore) supports both GCS and S3). This allows users to provide a start and end ledger range. The commands in this category export a list of everything that occurred within the provided range. All of the ranges are inclusive.
...
| datastore-endpoint-url | Optional datastore endpoint URL. Only used when --datastore-type is S3                 | ---                     |
...
Example command to read from Goldsky's public S3 bucket:

```bash
> stellar-etl export_ledgers -s 64334800 -e 64334810 --datastore-type S3 \
--datastore-region us-east-2 --datastore-path aws-public-blockchain/v1.1/stellar/ledgers \
-o exported_ledgers_s3.txt
```
