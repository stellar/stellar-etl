	...
	tests := []cliTest{
		{
			...
			args:    []string{"export_orderbooks", "-x", coreExecutablePath, "--stdout", "-s", "100000", "--stdout"},
			golden:  "",
			wantErr: fmt.Errorf("stellar-core needs a config file path when exporting ledgers continuously (endNum = 0)"),
		},
		{
			name:    "0 batch size",
			args:    []string{"export_orderbooks", "-b", "0", "-x", coreExecutablePath, "-c", coreConfigPath, "--stdout", "-s", "100000", "-e", "164000", "--stdout"},
			golden:  "",
			wantErr: fmt.Errorf("batch-size (0) must be greater than 0"),
		},
		{
			name:    "orderbook from single ledger",
			args:    []string{"export_orderbooks", "-x", coreExecutablePath, "-c", coreConfigPath, "-s", "5000000", "-e", "5000000", "--stdout"},
			golden:  "single_ledger.golden",
			wantErr: nil,
		},
		{
			name:    "orderbooks from large range",
			args:    []string{"export_orderbooks", "-x", coreExecutablePath, "-c", coreConfigPath, "--stdout", "-s", "6000000", "-e", "6001000", "--stdout"},
