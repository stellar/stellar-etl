...
func AddCommonFlags(flags *pflag.FlagSet) {
	...
	flags.String("datastore-endpoint-url", "", "Optional datastore endpoint URL. Only used when --datastore-type is S3.")
...
func BuildDataStoreConfig(env EnvironmentDetails) (datastore.DataStoreConfig, error) {
	...
	dataStoreType := strings.ToUpper(env.CommonFlagValues.DatastoreType)
	if dataStoreType == "" {
		dataStoreType = "GCS"
	}
	...
	case "GCS":
		// Keep params unchanged for the GCS path.
	case "S3":
	...
	default:
		return datastore.DataStoreConfig{}, fmt.Errorf("unsupported datastore type %q (accepted values: GCS, S3)", env.CommonFlagValues.DatastoreType)
	}
...
// CreateDatastore creates the datastore to interface with ledger txmeta files.
