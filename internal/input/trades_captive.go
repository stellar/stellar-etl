...
type TradeTransformInputCaptive struct {
...
func GetTradesCaptive(start, end uint32, limit int64, env utils.EnvironmentDetails) ([]TradeTransformInput, error) {
	...
	for seq := start; seq <= end; seq++ {
		...
		for int64(len(tradeSlice)) < limit || limit < 0 {
			...
			for index, op := range tx.Envelope.Operations() {
				...
				*/
				if operationResultsInTradeCaptive(op) && tx.Result.Successful() {
...
func operationResultsInTradeCaptive(operation xdr.Operation) bool {
