...
func (o *OrderbookParser) convertOffer(allConvertedOffers []transform.NormalizedOfferOutput, index int, offer ingest.Change, seq uint32, wg *sync.WaitGroup, lcm xdr.LedgerCloseMeta) {
	defer wg.Done()
	transformed, err := transform.TransformOfferNormalized(offer, seq, lcm)
	if err != nil {
...
func (o *OrderbookParser) parseOrderbook(orderbook []ingest.Change, seq uint32, lcm xdr.LedgerCloseMeta) {
	var group sync.WaitGroup
	allConverted := make([]transform.NormalizedOfferOutput, len(orderbook))
	for i, v := range orderbook {
		group.Add(1)
		go o.convertOffer(allConverted, i, v, seq, &group, lcm)
	}
...
func ReceiveParsedOrderbooks(orderbookChannel chan OrderbookBatch, logger *utils.EtlLogger, lcm xdr.LedgerCloseMeta) *OrderbookParser {
	...
	for {
		...
		case batch, ok := <-orderbookChannel:
			...
			for seq, orderbook := range batch.Orderbooks {
				batchParser.parseOrderbook(orderbook, seq, lcm)
			}
