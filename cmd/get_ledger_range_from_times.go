...
func init() {
	...
	getLedgerRangeFromTimesCmd.Flags().String("datastore-type", "GCS", "Datastore type to read txmeta files from. Accepted types are GCS and S3 (case-insensitive). Defaults to GCS.")
	getLedgerRangeFromTimesCmd.Flags().String("datastore-region", "", "Datastore region. Required when --datastore-type is S3; ignored for GCS.")
	getLedgerRangeFromTimesCmd.Flags().String("datastore-endpoint-url", "", "Optional custom datastore endpoint URL. Only used for S3-compatible datastores.")
